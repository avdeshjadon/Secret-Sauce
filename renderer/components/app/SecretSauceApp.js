...
export class SecretSauceApp extends LitElement {
    ...
    async _loadFromStorage() {
        try {
            ...
            const transparency = prefs.backgroundTransparency ?? 1.0;
            const colors = secretSauce.theme.get(themeName);
            secretSauce.theme.applyBackgrounds(colors.background, transparency, themeName);
    ...
    async handleStart() {
        ...
        if (providerMode === 'cloud') {
            ...
            // BYOK Mode - support Gemini, Groq, or OpenRouter
            const creds = await secretSauce.storage.getCredentials();
            const prefs = await secretSauce.storage.getPreferences();
            ...
            // Backend handles the routing based on keys/prefs
            await secretSauce.initializeBYOK(this.selectedProfile, this.selectedLanguage);
        }
