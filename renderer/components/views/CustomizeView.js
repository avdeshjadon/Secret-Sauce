...
export class CustomizeView extends LitElement {
    ...
    getThemes() {
        return window.secretSauce?.theme?.getAll() || [];
    }

    async _loadFromStorage() {
        try {
            const [prefs, keybinds] = await Promise.all([
                window.secretSauce?.storage?.getPreferences() || {},
                window.secretSauce?.storage?.getKeybinds() || null
            ]);
            
            this.googleSearchEnabled = prefs.googleSearchEnabled ?? true;
            this.backgroundTransparency = prefs.backgroundTransparency ?? 0.8;
            
            if (typeof prefs.fontSize === 'string') {
                this.fontSize = 20;
            } else {
    ...
    getDefaultKeybinds() {
        const isMac = window.secretSauce?.isMacOS || navigator.platform.includes('Mac');
        return {
    ...
    updateBackgroundAppearance() {
        ...
        secretSauce.theme.applyBackgrounds(colors.background, this.backgroundTransparency, this.theme);
    }
