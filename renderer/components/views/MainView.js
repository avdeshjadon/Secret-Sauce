...
export class MainView extends LitElement {
    ...
    async _loadFromStorage() {
        try {
            const [prefs, creds] = await Promise.all([
                secretSauce.storage.getPreferences(),
                secretSauce.storage.getCredentials().catch(() => ({})),
            ]);
            ...
            if (storedMode === 'cloud') {
                await secretSauce.storage.updatePreference('providerMode', this._mode);
            }

            // Load keys
            this._token = creds.cloudToken || '';
            this._geminiKey = (await secretSauce.storage.getApiKey().catch(() => '')) || '';
            this._groqKey = (await secretSauce.storage.getGroqApiKey().catch(() => '')) || '';
            this._openaiKey = creds.openaiKey || '';

            // Forced to BYOK mode
            this._mode = 'byok';

            // Load local AI settings (only Whisper)
            this._whisperModel = prefs.whisperModel || 'Xenova/whisper-small';
            this._openRouterKey = (await secretSauce.storage.getOpenRouterApiKey().catch(() => '')) || '';
            this._openRouterModel = prefs.openRouterModel || '';
    ...
    async _saveMode(mode) {
        ...
        this._tokenError = false;
        this._keyError = false;
        await secretSauce.storage.updatePreference('providerMode', mode);
        this.requestUpdate();
    }
    ...
    async _saveToken(val) {
        ...
        this._tokenError = false;
        try {
            const creds = await secretSauce.storage.getCredentials().catch(() => ({}));
            await secretSauce.storage.setCredentials({ ...creds, cloudToken: val });
        } catch (e) {}
        this.requestUpdate();
    }
    ...
    async _saveGeminiKey(val) {
        this._geminiKey = val;
        this._keyError = false;
        await secretSauce.storage.setApiKey(val);
        this.requestUpdate();
    }

    async _saveGroqKey(val) {
        this._groqKey = val;
        await secretSauce.storage.setGroqApiKey(val);
        this.requestUpdate();
    }
    ...
    async _saveOpenaiKey(val) {
        ...
        try {
            const creds = await secretSauce.storage.getCredentials().catch(() => ({}));
            await secretSauce.storage.setCredentials({ ...creds, openaiKey: val });
        } catch (e) {}
        this.requestUpdate();
    }
    ...
    async _saveOpenRouterKey(val) {
        ...
        await secretSauce.storage.setOpenRouterApiKey(val);
        if (val.trim()) {
            this._fetchOpenRouterModels();
        }
        this.requestUpdate();
    ...
    async _saveOpenRouterModel(val) {
        ...
        await secretSauce.storage.updatePreference('openRouterModel', val);
        this.requestUpdate();
    }

    async _fetchOpenRouterModels() {
        if (!this._openRouterKey) return;
        try {
            const models = await secretSauce.storage.fetchOpenRouterModels(this._openRouterKey);
            if (models && Array.isArray(models)) {
                // Sort free models to the top
                const sortedModels = models.sort((a, b) => {
                    const aFree = a.pricing && parseFloat(a.pricing.prompt) === 0;
                    const bFree = b.pricing && parseFloat(b.pricing.prompt) === 0;
                    if (aFree && !bFree) return -1;
                    if (!aFree && bFree) return 1;
                    return 0;
                });
                this._openRouterModels = sortedModels;

                // If no model selected yet, pick the first free one
                if (!this._openRouterModel && sortedModels.length > 0) {
                    this._openRouterModel = sortedModels[0].id;
                    await secretSauce.storage.updatePreference('openRouterModel', this._openRouterModel);
                }
            }
            this.requestUpdate();
        } catch (e) {
    ...
    _renderByokMode() {
        return html`
            ...
            <div class="form-group">
                ...
                <input type="password" placeholder="Optional" .value=${this._openRouterKey} @input=${e => this._saveOpenRouterKey(e.target.value)} />
                <div class="form-hint">
                    <span class="link" @click=${() => this.onExternalLink('https://openrouter.ai/keys')}>Get OpenRouter key</span>
                </div>
            </div>

            ${this._openRouterKey && this._openRouterModels.length > 0
                ? html`
                      <div class="form-group">
                          <label class="form-label">OpenRouter Model</label>
                          <select .value=${this._openRouterModel} @change=${e => this._saveOpenRouterModel(e.target.value)}>
                              ${this._openRouterModels.map(
                                  model => html`
                                      <option value=${model.id} ?selected=${this._openRouterModel === model.id}>
                                          ${model.name} ${parseFloat(model.pricing.prompt) === 0 ? '(Free)' : ''}
                                      </option>
                                  `
                              )}
                          </select>
    ...
    render() {
        return html`
            <div class="form-wrapper">
                <div class="page-title">Secret Sauce <span class="mode-suffix">BYOK</span></div>
                <div class="page-subtitle">Bring your own API keys</div>
